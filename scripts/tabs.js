	...
	setTimeout(function(){
		...
		if (schedule_update_data > 0){
			...
			$('.tab').each(function(){
				if (bg.tabs[this.id]){
					bg.tabs[this.id].n = $(this).index();
					if ($(this).parent().is('#tab_list')){
						bg.tabs[this.id].p = 'tab_list';
					} else {
						bg.tabs[this.id].p = $(this).parent()[0].id.substr(2);
					}
					if ($(this).is('.n')){
						bg.tabs[this.id].o = "n";
					} else {
...
function AppendTab(p){
	...
	div_tab.innerHTML = 
		...
		'<div class=tab_header id=tab_header'+p.tab.id+' draggable=true>'+
			...
			'<div class=drag_enter_center id=dc'+p.tab.id+'></div>'+
			'<div class=drag_entered_top id=du'+p.tab.id+'></div>'+
			'<div class=drag_entered_bottom id=dd'+p.tab.id+'></div>'+
			'<div class=close id=close'+p.tab.id+'>'+
				'<div class=close_img id=close_img'+p.tab.id+'></div>'+
			'</div>'+
	...
	if ($('#'+p.ParentId).is('.tab') && !p.tab.pinned){
