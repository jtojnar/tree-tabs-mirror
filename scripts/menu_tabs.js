...
function SetMenu(){
	...
	$(document).on("mousedown", "#pin_list, #tab_list, .tab, .pin", function(event){
		...
		if (event.button == 2){
			...
			$("#tabs_menu_mute, #tabs_menu_unmute").css({"display":"none"});
			if ($("#"+menuTabId).is(".muted")){
				$("#tabs_menu_mute").next().css({"display":"none"});
				$("#tabs_menu_unmute").next().css({"display":""});
			} else {
				$("#tabs_menu_mute").next().css({"display":""});
				$("#tabs_menu_unmute").next().css({"display":"none"});
			}
